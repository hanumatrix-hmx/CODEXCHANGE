...
export default async function SignupPage({
    ...
    const signUp = async (formData: FormData) => {
        ...
        // Robust origin detection: Vercel runtime vars first (per-deployment,
        // not baked at build time), then x-forwarded-host, then env fallback.
        let origin: string;
        const forwardedHost = headersList.get("x-forwarded-host");
        if (forwardedHost) {
            origin = `${forwardedHost.includes("localhost") ? "http" : "https"}://${forwardedHost}`;
        ...
        if (error) {
            ...
            const errorMessage = error.message.toLowerCase().includes("rate limit")
                ...
                : error.message;
            return redirect(`/signup?message=Could not create account: ${errorMessage}`);
        }

        return redirect("/signup?message=Check your email for magic link to complete signup");
    };
            ...
            <div className="relative z-10 w-full max-w-md space-y-8 bg-white/50 dark:bg-slate-900/50 backdrop-blur-xl border border-slate-200/50 dark:border-white/10 p-8 rounded-2xl shadow-xl">
                ...
                <div className="mt-8 space-y-6">
                    <div className="flex flex-col gap-4">
                        {/* OAuth buttons use anchor tags → /api/auth/[provider] route handler.
                            This is intentional: server actions cannot reliably redirect to
                            external OAuth URLs or flush PKCE cookies. The route handler does both. */}
                        <Link
